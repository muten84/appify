...
public class WebAppGenerator extends Generator {
	...
	public String generate(TreeLogger logger, GeneratorContext context, String typeName)
		...
		try {
			...
			if (pw != null) {
				...
				if (offline != null) {
					webappBuilder = addOfflineApplicationCacheService(webappBuilder, offline);
				}
				webappBuilder = overrideStoreCurrentAppState(webappBuilder, modelClass);
	...
	protected TypeSpec.Builder overrideStoreCurrentAppState(TypeSpec.Builder webappBuilder, Class<?> webAppModel) {
		return webappBuilder.addMethod(MethodSpec.methodBuilder("storeCurrentAppState").addAnnotation(Override.class)
	...
	protected TypeSpec.Builder addStorageService(TypeSpec.Builder spec, Storage annotation,
		...
		if (storageType.equals(it.appify.api.Storage.LOCAL_STORAGE)) {
			TypeSpec storage = TypeSpec.anonymousClassBuilder("").superclass(LocalStorage.class)
					.addField(FieldSpec.builder(ParameterizedTypeName.get(ObjectMapper.class, storageModelType),
							"mapper", Modifier.PRIVATE).build())

			.addMethod(MethodSpec.methodBuilder("getObjectMapper").addModifiers(Modifier.PROTECTED)
					.returns(ParameterizedTypeName.get(ObjectMapper.class, storageModelType))
					.addAnnotation(Override.class)
					.addStatement("if(mapper==null){mapper = $T.create($N.class);}return mapper", GWT.class,
							objectMapperInterface.name)
					.build()).build();

			spec.addMethod(MethodSpec.methodBuilder("getStorageService").addModifiers(Modifier.PUBLIC)
					.returns(it.appify.api.Storage.class).addCode("return $L;", storage).build());
			return spec;
	...
	protected Offline scanOfflineService(JClassType classType) {
		Offline annotation = classType.findAnnotationInTypeHierarchy(Offline.class);
		if (annotation != null) {
			return annotation;
		}
		return null;
	...
	protected TypeSpec.Builder addGeolocationService(TypeSpec.Builder spec, Geolocation annotation) {
		...
		MethodSpec.Builder methodBuilder = MethodSpec.methodBuilder("getGeolocationService")
				.addModifiers(Modifier.PUBLIC).addAnnotation(Override.class).returns(it.appify.api.Geolocation.class);

		switch (type) {
		case Geolocation.HTML5:
			methodBuilder.addCode("return $T.createGeoLocationService($L,$L,$L);", ServiceProvider.class, highAccuracy,
					maxAge, timeout);
			break;
	...
	protected TypeSpec.Builder addScreenOrientationService(TypeSpec.Builder spec, ScreenOrientation annotation) {
		...
		spec.addMethod(MethodSpec.methodBuilder("getScreenOrientationService").addModifiers(Modifier.PUBLIC)
				.addAnnotation(Override.class).returns(it.appify.screenorientation.WebScreenOrientation.class)
				.addCode("return $T.createWebScreenOrientation();", ServiceProvider.class).build());
		return spec;
	}

	protected TypeSpec.Builder addBatteryService(TypeSpec.Builder spec, Battery annotation) {

		spec.addMethod(MethodSpec.methodBuilder("getBatteryService").addModifiers(Modifier.PUBLIC)
				.addAnnotation(Override.class).returns(it.appify.api.Battery.class)
	...
	protected TypeSpec.Builder createWebAppClass(String className, String mainPage, Class<?> modelClass,
			Class<?> superInterface, TypeSpec objectMapperInterface) throws ClassNotFoundException {

		TypeSpec.Builder webAppbuilder = TypeSpec.classBuilder(className).addModifiers(Modifier.PUBLIC, Modifier.FINAL)
				.addSuperinterface(superInterface)
				.superclass(ParameterizedTypeName.get(AbstractWebApp.class, modelClass)).addType(objectMapperInterface)
				.addMethod(createDefaultConstructor(mainPage))
				// .addMethod(createScanDynamicContent(container))
				.addMethod(createSuperClassMethods(modelClass, objectMapperInterface))
	...
	protected MethodSpec initializeServices(TypeOracle typeOracle) {
		...
		for (JClassType jClassType : types) {
			...
			try {
				...
				TypeSpec serviceHandler = createServiceLifeCycleHandlers("service" + jClassType.getSimpleSourceName(),
						serviceAnnotation.name(), startMethodName, stopMethoName);
			...
			for (JField jField : fields) {
				...
				if (jField.isPublic()) {
					if (el != null) {
						...
						TypeSpec.Builder innerControllerBuilder = createControllerHolderBuilder(controllerClass, pageId,
								elId, jField.getName(), "controller" + jClassType.getSimpleSourceName(), false);
					...
					if (el != null) {
						...
						initializeControllerBuilder.addStatement("this.bindElementToPage($S,$S)", pageId, elId);
						TypeSpec.Builder innerControllerBuilder = createControllerHolderBuilder(controllerClass, pageId,
								elId, jField.getName(), "controller" + jClassType.getSimpleSourceName(), true);
						controllerHolderSpecs.put("controller" + jClassType.getSimpleSourceName() + "##" + pageId,
								innerControllerBuilder);
						initializeControllerBuilders.put("controller" + jClassType.getSimpleSourceName(),
								initializeControllerBuilder);
			...
			for (JMethod jMethod : methods) {
				...
				if (vhAnnotation != null) {
					...
					TypeSpec innerViewHandler = createViewHandler("controller" + jClassType.getSimpleSourceName(),
							jMethod.getName());
					initializeControllerBuilder.addStatement(
							"$T holder" + pageId + "_" + viewId + " = this.createViewHandler(\"$L\",\"$L\",\"$L\",$L)",
	...
	private TypeSpec createViewModelHandler(JMethod method, Class<?> modelType, String controllerRef) {
		return TypeSpec.anonymousClassBuilder("")
				.addSuperinterface(ParameterizedTypeName.get(it.appify.api.HasModel.ViewModelHandler.class, modelType))
				.addMethod(
						MethodSpec.methodBuilder("onEvent").addModifiers(Modifier.PUBLIC).addAnnotation(Override.class)
								.addParameter(String.class, "eventType").addParameter(String.class, "pageId")
								.addParameter(String.class, "sourceElementId").addParameter(modelType, "model")
								.addStatement(controllerRef + "." + method.getName() + "(model)").build())
				.build();
	}

	private TypeSpec createViewHandlerHolder(Class<?> modelType, TypeSpec vmHandler) {
		TypeSpec omi = objectMappersInterfaces.get(modelType);
		TypeSpec.Builder vmHandlerBuilder = TypeSpec.anonymousClassBuilder("$L", vmHandler)
				.superclass(ParameterizedTypeName.get(ViewModelHandlerHolder.class, modelType))

		.addField(FieldSpec
				.builder(ParameterizedTypeName.get(ObjectMapper.class, modelType), "mapper", Modifier.PRIVATE).build())
				.addMethod(MethodSpec.methodBuilder("getObjectMapper").addModifiers(Modifier.PROTECTED)
						.returns(ParameterizedTypeName.get(ObjectMapper.class, modelType)).addAnnotation(Override.class)
						.addStatement("if(mapper==null){mapper = $T.create($N.class);}return mapper", GWT.class,
								omi.name)
						.build());
	...
	private TypeSpec createServiceLifeCycleHandlers(String serviceRef, String serviceName, String startMethodName, String stopMethodName) {
		TypeSpec.Builder serviceHandlerBuilder = TypeSpec.anonymousClassBuilder("").addSuperinterface(it.appify.api.Service.class);
				if(startMethodName!=null){
					serviceHandlerBuilder.addMethod(MethodSpec.methodBuilder("start").addModifiers(Modifier.PUBLIC)
						// .returns(Void.class)
						.addStatement(serviceRef + "." + startMethodName + "()")
						.addStatement("$T.register($S,this)", ServiceManager.class, serviceName)
						.build());
				...
				if(stopMethodName!=null){
					serviceHandlerBuilder.addMethod(MethodSpec.methodBuilder("stop").addModifiers(Modifier.PUBLIC)
						.addStatement(serviceRef + "." + stopMethodName + "()").build());
				}else{
					serviceHandlerBuilder.addMethod(MethodSpec.methodBuilder("stop").addModifiers(Modifier.PUBLIC)
							.build());
				}
				return serviceHandlerBuilder.build();		
	}

	private TypeSpec.Builder callPageReadyMethod(TypeSpec.Builder controllerHolderSpec, String pageReadyHandlerMethod) {
		MethodSpec.Builder callPageReadyMethodBuilder = MethodSpec.methodBuilder("callPageReadyHandler")
				.addAnnotation(Override.class).addModifiers(Modifier.PUBLIC);
		if (pageReadyHandlerMethod != null && pageReadyHandlerMethod.length() > 0) {
			callPageReadyMethodBuilder.addStatement("this.controller.$L()", pageReadyHandlerMethod);
		}
		controllerHolderSpec.addMethod(callPageReadyMethodBuilder.build());
		return controllerHolderSpec;
	}

	private TypeSpec createControllerHolder(TypeSpec.Builder controllerHolderBuilder) {
	...
	private TypeSpec.Builder createControllerHolderBuilder(Class<?> controllerClass, String pageId, String viewId,
			String fieldName, String controller, boolean useSetter) {

		TypeSpec.Builder controllerHolderBuilder = TypeSpec
				.anonymousClassBuilder("$S,$S,$S,$L", pageId, viewId, fieldName, controller)
				.superclass(ParameterizedTypeName.get(ControllerHolder.class, controllerClass));
	...
	private TypeSpec createViewHandler(String controllerRef, String methodName) {
		TypeSpec viewHandler = TypeSpec.anonymousClassBuilder("")
				.superclass(it.appify.api.HasViewHandlers.ViewHandler.class)
				.addMethod(MethodSpec.methodBuilder("onEvent").addModifiers(Modifier.PUBLIC)
						.addParameter(String.class, "type").addParameter(String.class, "source")
						.addStatement(controllerRef + "." + methodName + "()").build())
				.build();
		return viewHandler;
	}

	protected MethodSpec createSuperClassMethods(Class<?> modelClass, TypeSpec objectMapperInterface) {
		return MethodSpec.methodBuilder("getAppStateModelView").addModifiers(Modifier.PUBLIC, Modifier.FINAL)
				.addAnnotation(Override.class).returns(ParameterizedTypeName.get(WebModelView.class, modelClass))
				.addStatement("return $L", createViewModelClass(modelClass, objectMapperInterface)).build();
	}

	private TypeSpec createViewModelClass(Class<?> modelClass, TypeSpec objectMapperInterface) {

		TypeSpec viewModel = TypeSpec.anonymousClassBuilder("")
				.superclass(ParameterizedTypeName.get(VueJsViewModel.class, modelClass))
				.addField(FieldSpec
						.builder(ParameterizedTypeName.get(ObjectMapper.class, modelClass), "mapper", Modifier.PRIVATE)
						.build())

		.addMethod(MethodSpec.methodBuilder("getObjectMapper").addModifiers(Modifier.PROTECTED)
				.returns(ParameterizedTypeName.get(ObjectMapper.class, modelClass)).addAnnotation(Override.class)
				.addStatement("if(mapper==null){mapper = $T.create($N.class);}return mapper", GWT.class,
						objectMapperInterface.name)
				.build()).build();
		return viewModel;
	...
	private TypeSpec createObjectMapperInterface(String interfaceName, Class<?> modelClass) {
		TypeSpec interfaceMapper = TypeSpec.interfaceBuilder(interfaceName)
				.addModifiers(Modifier.PUBLIC, Modifier.STATIC)
				.addSuperinterface(ParameterizedTypeName.get(ObjectMapper.class, modelClass)).build();
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		TypeSpec type = generator.createWebAppClass("MyWebAppImpl", "mainPage", Object.class, Runnable.class, null)
				.build();
