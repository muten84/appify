...
public class WebAppGenerator extends Generator {
	...
	protected TypeSpec.Builder overrideStoreCurrentAppState(TypeSpec.Builder webappBuilder, Class<?> webAppModel) {
		return webappBuilder.addMethod(MethodSpec.methodBuilder("storeCurrentAppState").addAnnotation(Override.class).addModifiers(Modifier.PROTECTED).addCode("if(getStorageService()!=null){" + "$T model = ($T) getCurrentAppState();" + "getStorageService().store($S,model);" + "}", webAppModel, webAppModel, webAppModel.toString()).build());
	}
	...
	protected TypeSpec.Builder addOfflineApplicationCacheService(TypeSpec.Builder spec, Offline annotation) {
		...
		spec.addMethod(MethodSpec.methodBuilder("getApplicationCacheService").addModifiers(Modifier.PUBLIC).addAnnotation(Override.class).returns(ApplicationCache.class).addCode("return $T.createApplicationCache();", ServiceProvider.class).build());
		return spec;
	}
			...
			for (JMethod jMethod : methods) {
				...
				if (vhAnnotation != null) {
					...
					if (objectMappersInterfaces.get(modelType) == null) {
						TypeSpec objectMapperInterface = createObjectMapperInterface("ObjectMapper" + modelType.getSimpleName(), modelType);
						objectMappersInterfaces.put(modelType, objectMapperInterface);
					}
					TypeSpec vmHandler = createViewModelHandler(jMethod, modelType, "controller" + jClassType.getSimpleSourceName());
					TypeSpec vmh = createViewHandlerHolder(modelType, vmHandler);
					initializeControllerBuilder.addStatement("this.modelView.addViewModelHandler($S,$S,$L)", pageId, viewId, vmh);
	...
	private TypeSpec createViewModelHandler(JMethod method, Class<?> modelType, String controllerRef) {
		return TypeSpec.anonymousClassBuilder("").addSuperinterface(ParameterizedTypeName.get(it.appify.api.HasModel.ViewModelHandler.class, modelType)).addMethod(MethodSpec.methodBuilder("onEvent").addModifiers(Modifier.PUBLIC).addAnnotation(Override.class).addParameter(String.class, "eventType").addParameter(String.class, "pageId").addParameter(String.class, "sourceElementId").addParameter(modelType, "model").addStatement(controllerRef + "." + method.getName() + "(model)").build()).build();
	}
	...
	private TypeSpec createViewHandlerHolder(Class<?> modelType, TypeSpec vmHandler) {
		...
		TypeSpec.Builder vmHandlerBuilder = TypeSpec.anonymousClassBuilder("$L", vmHandler).superclass(ParameterizedTypeName.get(ViewModelHandlerHolder.class, modelType))

		.addField(FieldSpec.builder(ParameterizedTypeName.get(ObjectMapper.class, modelType), "mapper", Modifier.PRIVATE).build()).addMethod(MethodSpec.methodBuilder("getObjectMapper").addModifiers(Modifier.PROTECTED).returns(ParameterizedTypeName.get(ObjectMapper.class, modelType)).addAnnotation(Override.class).addStatement("if(mapper==null){mapper = $T.create($N.class);}return mapper", GWT.class, omi.name).build());
