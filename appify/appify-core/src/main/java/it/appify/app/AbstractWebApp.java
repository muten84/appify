...
public abstract class AbstractWebApp<AppState> implements WebApp<AppState> {
	...
	public void onPageReady(Page<Element> page) {
	    ConsoleLogger.getConsoleLogger().log(
		    "AbstractWebApp onPageReady: " + mainPage + " - "
		...
		for (ControllerHolder<?> controllerHolder : controllers) {
		    ConsoleLogger.getConsoleLogger().log(
			    "injecting elements: " + controllerHolder.pageId
				    + " - " + controllerHolder.viewId + " - "
				    + controllerHolder.fieldName);
		    controllerHolder.injectViewElements();
		}
		...
		ConsoleLogger.getConsoleLogger().log(
			"warning control holders injectViewElements no elements: "
		...
		firstLoad = false;
		ConsoleLogger.getConsoleLogger().log(
			"Going to starting all services...");
		...
		if (ctrlHolders != null) {
		    ...
		    ConsoleLogger.getConsoleLogger().log(
			    "warning callPageReadyHandler no elements: "
				    + ctrlHolders + " - " + page.getPageId());
		}
	    }
	...
	scandDynamicContent(new ContentLoadedListener() {
	    ...
	    public void error() {
		ConsoleLogger.getConsoleLogger().log(
			"error while scandDynamicContent ");
	...
	for (Element element : els) {
	    if (element.getAttribute("appify-page") != null
		    && element.getAttribute("appify-page").equals(pageId)) {
		elRef = element;
		break;
	    }
	}
